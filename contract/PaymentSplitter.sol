...
contract PaymentSplitter is Context {
    ...
    constructor(address[] memory payees, uint256[] memory shares_) payable {
        ...
        //Opensea Polygon NFT can use wrapped ETH
     ...
     * contract.
     */
    function releaseToken(IERC20 token, address account) public virtual {
        require(_shares[account] > 0, "PaymentSplitter: account has no shares");
    ...
    function _addPayee(address account, uint256 shares_) internal virtual {
        ...
        require(payeeExists == 0, "PaymentSplitter: payee already added");
    ...
    function _setShares(address account, uint256 shares_) internal virtual {
        require(account != address(0), "PaymentSplitter: account is the zero address");
        require(shares_ >= 0, "PaymentSplitter: shares must be 0 or greater than 0");
        ...
        //pay out before changing shares to ensure calculations are correct
        withdraw();
    ...
    function _addToken(address token) internal virtual {
        require(token != address(0), "PaymentSplitter: token is the zero address");
    ...
    function withdraw() public virtual {
        ...
        for (uint256 i = 0; i < _payees.length; i++) {
            ...
            // if the payee has shares
            if (_shares[account] > 0) {
                //loop list of tokens
                for (uint256 j = 0; j < _tokenList.length; j++) {
                    IERC20 token = _tokenList[j];
                    if (token.balanceOf(address(this)) > 0) {
                        totalReceived = token.balanceOf(address(this)) + totalReleasedToken(token);
                        payment = _pendingPayment(account, totalReceived, releasedToken(token, account));

                        //check if token is due and send it
                        if (payment > 0) {
                            releaseToken(token, account);
                        }
                    }
                }
                ...
                if (address(this).balance > 0) { 
                    ...
                    //check if ETH payment is due and send it
                    if (payment > 0) {
